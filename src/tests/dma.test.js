...
describe('DMA', () => {
  ...
  test('dma_in (sync) with explicit size', async () => {
  ...
  test('dma_out_async with explicit size', async () => {
    const {asm, warn} = await transpileSource(`function test() {
      u32<$s4> dest = 0x1000;
      u32<$s0> rdram = 0x2000;
      u32<$t0> size = 64;
  ...
  test('dma_in_async with memory dest (2-arg, auto size)', async () => {
...
function test() {
  ...
  test('dma_in_async with register dest (3-arg)', async () => {
    const {asm, warn} = await transpileSource(`function test() {
      u32<$s4> dest = 0x1000;
      u32<$s0> rdram = 0x2000;
      u32<$t0> size = 64;
